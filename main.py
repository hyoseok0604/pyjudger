...
if __name__ == "__main__":
    ...
    _, language, time_limit, memory_limit, code_path, *testcase_paths = sys.argv
    ...
    try:
        ...
        print_exit({"message": "오류 발생"})


    compile_result = compile(code_path, "python3 -m py_compile {}".format(language))

    if not compile_result:
        print_exit({"result": "컴파일 에러"})

    output_path = os.path.join(code_path, 'a.out')
    systemcall_names = get_systemcall_names()

    max_time = 0
    max_memory = 0

    for testcase_input_path, testcase_output_path in zip(testcase_paths[::2], testcase_paths[1::2]):
        testcase_result, testcase_memory, testcase_time = run_testcase(
            code_path,
            "python3 {}".format(language),
            testcase_input_path,
